...
from fancyclock.version import APP_APPUSERMODELID
...
def _default_icon_location_for(target_exe: Path) -> str:
    ...
    Prefer `fancyclock.ico` next to the exe (deployed by the installer) so the
    shortcut uses the branded icon even if the exe's embedded icon changes.
    """

    try:
        ico = target_exe.resolve().parent / DEPLOYED_ICO_NAME
